...
const LikeButton = (props: {
  isLiked: boolean;
  isLikedChangeHandler: () => Promise<boolean>;
  fetching: boolean;
}) => {
  ...
  const [fill, setFill] = useState(props.isLiked ? likedFill : notLikedFill);
  const [path, setPath] = useState(props.isLiked ? likedPath : notLikedPath);
  ...
  const onLike = async () => {
    ...
    const success = await props.isLikedChangeHandler();
